...
export default {
  ...
  async fetch() {
    ...
    const { data } = await this.fetchSinglePost(this.$route.params.id)
    this.oldImage = data.data.thumbnail
    this.fillForm()
    this.$loading().close()
  },
  data() {
  ...
  computed: {
    ...mapState({
      viewing: (state) => state.post.viewing,
    }),
  },
  watch: {},
  methods: {
    ...mapActions({
      updatePost: postActions.UPDATE.SINGLE,
      fetchTag: tagActions.FETCH.DATA,
      fetchCategories: categoryActions.FETCH.DATA,
      fetchSinglePost: postActions.FETCH.SINGLE,
    }),
    async onSubmit() {
      try {
        ...
        if (this.oldImage !== this.form.thumbnail) {
    ...
    fillForm() {
      ...
      this.imageList.push({ url: this.viewing.data.thumbnail })
      // JSON.parse(
      //   JSON.stringify(this.viewing.data.product_categories)
      // ).forEach((e) => this.form.product_categories.push(e.id))
      const Ids = this.viewing.data.categories.map((e) => e.id)
      this.form.categories = Ids
      const Ids2 = this.viewing.data.tags.map((e) => e.id)
      this.form.tags = Ids2
    },
  },
  head() {
