...
export default {
  ...
  created() {
    this.fillFormFromQuery(this.$store.state.product.category.query)
  },
  methods: {
    handleFilter() {
      if (this.filter) {
        this.$emit('my-product-category-filter', this.filter)
      }
    },
    resetFilter() {
      this.form = {
      ...
      this.filter = null
      // this.form.handle()
      this.$refs.productCategoryFilterForm.reset()
      this.$emit('my-product-category-filter-refresh')
    },
  },
}
</script>
